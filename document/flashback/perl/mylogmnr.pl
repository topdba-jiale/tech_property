		   ...
		   if ($_ =~ m/^###   @([0-9]+)=(.*)/s ) {
			   ...
			   if ($col == 0) {
			   		if($type eq "UPDATE" && $updold==1){
			   			$sql = "UPDATE $tabname";
			   			$wheresql=" WHERE ${$tabhash{$tabname}}[$varcol]=$var";
			   			$undosql="UPDATE $tabname SET ";
			   			$undo_setsql="${$tabhash{$tabname}}[$varcol]=$var";
			   		}elsif($type eq "DELETE"){
			   			$sql = "DELETE FROM $tabname where ${$tabhash{$tabname}}[$varcol]=$var ";
			   			$undosql = "INSERT INTO $tabname  VALUES( $var ";
			   		}elsif($type eq "INSERT"){
			   			$sql = "INSERT INTO $tabname  VALUES( $var ";
			   			$undosql = "DELETE FROM $tabname where ${$tabhash{$tabname}}[$varcol]=$var ";
			   		}
				  
			   } else { 
			   		if($type eq "UPDATE" && $updold==1){
			   			$wheresql=$wheresql . " and ${$tabhash{$tabname}}[$varcol]=$var";
		   				...
		   				if($undo_wheresql eq " WHERE "){
		   					$undo_wheresql=$undo_wheresql . " ${$tabhash{$tabname}}[$varcol]=$var";
		   				}else{
		   					$undo_wheresql=$undo_wheresql . " and ${$tabhash{$tabname}}[$varcol]=$var";
		   				}
		   				
			   		}elsif($type eq "DELETE"){
			   			$sql = $sql . " and ${$tabhash{$tabname}}[$varcol]= $var";
			   			$undosql = $undosql . ", $var";
			   		}elsif($type eq "INSERT"){
			   			$sql = $sql . ", $var";
			   			$undosql = $undosql . " and ${$tabhash{$tabname}}[$varcol]= $var";
			   		}
